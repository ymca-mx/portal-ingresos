$(function () {
    ...
    var CambioFn = {
        init() {
            ...
            $('#txtClave').on('keydown', this.ClaveKeydown);
            $("#btnCambio").on('click', this.CambioClick);
        ...
        Buscar() {
            ...
            IndexFn.Api("Alumno/CambioCarrera/" + AlumnoId + "/" + localStorage.getItem('userAdmin'), "get", "")
                .done(function (data) {
                    $('#lblNombre').text("");
        ...
        GuardarClick() {
            {
                ...
                var obj = JSON.stringify(lstop[0]);

                IndexFn.Api("Alumno/CambioCarrera", "POST", obj)
                    .done(function (data) {
                        IndexFn.Block(false);
                        $("#txtComentario").empty();
                        $("#slcOfertaEducativa").empty();
