...
var Usuario,
    ...
    var plugin = {
        init() {
            var $validar = form.validate({
                ...
                errorPlacement: function (error, element) { //Agrega el icono de warning en el campo input que haga falta
                    var icon = $(element).parent('.input-icon').children('i');
                    icon.removeClass('fa-check').addClass("fa-warning");
                    icon.attr("data-original-title", error.text()).tooltip({ 'container': 'body' });
                },
                highlight: function (element) { // Pinta de rojo a los campos faltantes
                    $(element)
            ...
            $('#RootWizard').bootstrapWizard({
                'tabClass': 'has-success',
                'onNext': function (tab, navigation, index) {

                    var $valid = form.valid();
                    if (!$valid) {
                        $($(navigation[0]).find('li.active')).removeClass("done");
                        $validar.focusInvalid();
                        return false;
                    }

                    if (index === 1) { $('#btnlist').hide(); }
                    else if (index === 2) {
                        $('#btnNext')[0].innerHTML = $('#btnNext')[0].innerHTML.replace('Siguiente', 'Guardar');
                    } else if (index === 3) {
                        success.show();
                        error.hide();
                        $($(navigation[0]).find('li.active')).addClass("done");
                        InscripcionFn.AddAlumno();
                        return false;
                    }
    ...
    var InscripcionFn = {
        init() {            
            ...
            $('#slcLugarN').on('change', this.setPaisEstado);//<<<--- falta 
            $('#btnGuardar').on('click', this.GuardarDescuentos);
        ...
        AddAlumno() {
            ...
            //$('#btnNext')[0].innerHTML = $('#btnNext')[0].innerHTML.replace('Guardar', 'Siguiente');
            //$('#btnGuardar').show();
        },
        Progress(porcentaje) {
            InscripcionFn.Porcentaje += porcentaje;

            $('#form_wizard_1').find('.progress-bar').css({
                width: (InscripcionFn.Porcentaje).toString() + '%'
            });
        },
        Porcentaje: 0,
        ActiveObse() {
        ...
        btnNoClick() {
            Metronic.scrollTo($('#chkEsEmpresa'));
        },
        btnPromocionClick() {
        ...
        SetFecha() {
            ...
            $("#divFecha .input-group").datepicker("update", Fecha);
