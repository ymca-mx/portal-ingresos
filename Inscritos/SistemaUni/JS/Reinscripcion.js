$(document).ready(function () {
    ...
    function DatosAlumno() {
        ...
        //var AlumnoId = '9579';
        $.ajax({
            url: '../WebServices/WS/Alumno.asmx/ConsultarAlumnoReinscripcion',
            type: 'POST',
    ...
    function Pagar(Descripcion) {
         ...
         $.ajax({
             type: "POST",
             url: "../WebServices/WS/Reinscripcion.asmx/GenerarInscrCole",
    ...
    $('#btnGenerar').on('click', function () {
        ...
        if (TipoOferta == 4) {
            $.ajax({
                type: "POST",
                url: "../WebServices/WS/Reinscripcion.asmx/ConsultarPagodeMes",
            ...
            $.ajax({
                type: "POST",
                url: "../../WebServices/WS/Reinscripcion.asmx/ConsultarPagosPeriodo",
                data: "{AlumnoId:" + AlumnoId + ",OfertaEducativaId:" + OfertaEducativa + "}", // the data in form-encoded format, ie as it would appear on a querystring
                //contentType: "application/x-www-form-urlencoded; charset=UTF-8", // if you are using form encoding, this is default so you don't need to supply it
                contentType: "application/json; charset=utf-8", // the data type we want back, so text.  The data will come wrapped in xml
                success: function (data) {
    ...
    function GenerarIngles(MesId) {
        $.ajax({
            type: "POST",
            url: "../../WebServices/WS/Reinscripcion.asmx/GenerarColegiaturaIngles",
    ...
    function Pendiente() {
        ...
        $.ajax({
            type: "POST",
            url: "../WebServices/WS/Reinscripcion.asmx/Pendiente",
                    ...
                    alertify.confirm("El alumno esta Inscrito pero no ha generado cargos" +
                        "¿Desea generar sus cargos del " + data.d[1] + "?", function (e) {
                            if (e) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: "../WebServices/WS/Reinscripcion.asmx/InscribirGenerar",
                                    data: '{AlumnoId:"' + AlumnoId + '",OfertaEducativaId:"' + OfertaEducativa + '"}',
                                    contentType: "application/json; charset=utf-8",
                                    success: function (data) {
                                        if (data.d == "Guardado") {
